...
class LLMEngine:
    ...
    def __init__(self, config: dict):
        ...
        for i in range(1, world_size):
            event = ctx.Event()
            process = ctx.Process(target=worker_process,
                                  args=(config, i, event))
    ...
    def step(self) -> tuple[list[int], bool]:
        ...
        outputs = self.model_runner.call(
            "run", scheduled_sequences, is_prefill)
        ...
        num_processed_tokens = sum(len(
            seq) for seq in scheduled_sequences) if is_prefill else len(scheduled_sequences)
    ...
    def generate(self, prompts: list[str], sampling_params: SamplingParams) -> list[str]:
        ...
        while not self.scheduler.is_finished():
            ...
            generated_tokens.update(
                {seq_id: tokens for seq_id, tokens in outputs})
        # sort generated tokens by seq_id to maintain order
        generated_tokens = [generated_tokens[seq_id]
                            for seq_id in sorted(generated_tokens.keys())]
        output = {'text': [self.tokenizer.decode(
            tokens) for tokens in generated_tokens], 'token_ids': generated_tokens}
        return output
