...
def _segment_reduce_weighted_kernel(
    # Input pointers
    # (num_sorted, d_model) - expert outputs sorted by target token
    expert_output_ptr,
    sorted_weights_ptr,     # (num_sorted,) - weights sorted by target token
    # (num_tokens + 1,) - CSR-style offsets for each token's segment
    segment_offsets_ptr,
    # Output pointer
...
def segment_reduce_weighted(
    # (num_sorted, d_model) - outputs sorted by target token
    expert_output: torch.Tensor,
    # (num_sorted,) - weights sorted by target token
    ...
    Segment offsets are computed analytically (each token has exactly top_k entries).
    ...
    segment_offsets = torch.arange(
        num_tokens+1, device=device, dtype=torch.int64) * top_k
    ...
    _segment_reduce_weighted_kernel[grid](
        ...
        y_flat,
        num_tokens, d_model,
        expert_output.stride(0), y_flat.stride(0),
        BLOCK_D=BLOCK_D,
        MAX_TOPK=MAX_TOPK,
    )
...
def fused_scatter_add_weighted(
    # (num_sorted, d_model) - raw expert outputs
    expert_output: torch.Tensor,
    # (num_sorted,) - target token indices (NOT sorted by target)
    sorted_token_idx: torch.Tensor,
    sorted_weights: torch.Tensor,   # (num_sorted,) - weights
    num_tokens: int,                # number of original tokens
    # experts per token (for optimization hints)
    top_k: int = 1,
) -> torch.Tensor:
    """
