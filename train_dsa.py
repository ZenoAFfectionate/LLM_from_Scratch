    ...
    def freeze_except_indexer(self):
        ...
        for layer_idx, att in self._dsa_layers:
            for name, param in att.indexer.named_parameters():
                ...
                print(
                    f"  Layer {layer_idx} indexer.{name}: requires_grad=True")
...
def train_warmup_step(
    ...
    with torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
        ...
        mask = torch.tril(torch.ones(
            (seq_len, seq_len), device=x.device, dtype=torch.bool)) if seq_len > 1 else None
        ...
        for idx, block in enumerate(model.layers):
            ...
            if hasattr(block.att, 'indexer'):
                kl_loss = block.att.compute_indexer_loss_dense(
                    x_norm, mask=mask)
                total_kl_loss = total_kl_loss + kl_loss
                attn_out, _ = block.att(x_norm, mask=mask, use_sparse=False)
            else:
                attn_out = block.att(x_norm, mask=mask)
    ...
    if accumulation_step == gradient_accumulation_steps - 1:
        ...
        if trainable_params is not None:
            grad_norm = clip_grad_norm_(
                trainable_params, config.get('max_grad_norm', 1.0)).item()
        else:
            grad_norm = clip_grad_norm_(
...
def run_warmup_stage(
    ...
    save_checkpoint(model, optimizer, warmup_steps,
...
def train_sparse_step(
    ...
    lm_grad_norm = 0.0
    if accumulation_step == gradient_accumulation_steps - 1:
        lm_grad_norm = clip_grad_norm_(
            main_params, config.get('max_grad_norm', 1.0)).item()
        main_optimizer.step()
    ...
    with torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
        ...
        for block in model.layers:
            ...
            with torch.no_grad():
                if hasattr(block.att, 'indexer'):
                    attn_out, _ = block.att(x_norm, mask=mask, use_sparse=True)
                else:
                    attn_out = block.att(x_norm, mask=mask)
                x = block.dropout(attn_out)
                x, residual = block.ffn_norm(x, residual)
    ...
    kl_grad_norm = 0.0
    if accumulation_step == gradient_accumulation_steps - 1:
        kl_grad_norm = clip_grad_norm_(
            indexer_params, config.get('max_grad_norm', 1.0)).item()
    ...
    for step in range(start_step, sparse_steps):
        ...
        if (step + 1) % eval_interval == 0:
            ...
            if val_loss < best_val_loss:
                ...
                save_checkpoint(model, main_optimizer,
                                step + 1, str(best_path))
...
def main():
    parser = argparse.ArgumentParser(
        description='Train DSA (DeepSeek Sparse Attention)')
    ...
    if config.attention_type != "DSA":
        print(
            f"Warning: attention_type is '{config.attention_type}', expected 'DSA'")
        print("DSA training requires DSA attention layers in the model.")

    model = TransformerLM(config=config, device=device,
